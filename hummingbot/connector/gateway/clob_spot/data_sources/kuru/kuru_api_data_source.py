...
from hummingbot.connector.gateway.clob_spot.data_sources.kuru.kuru_constants import (
    CONNECTOR,
...
from hummingbot.connector.gateway.clob_spot.data_sources.kuru.kuru_helpers import (
    AsyncLock,
    automatic_retry_with_timeout,
    generate_hash,
)
from hummingbot.connector.gateway.clob_spot.data_sources.kuru.kuru_types import OrderStatus as KuruOrderStatus
...
class KuruAPIDataSource(GatewayCLOBAPIDataSourceBase):
    ...
    async def start(self):
        ...
        await super().start()
        # TODO: Listen to stream here for last traded price
    ...
    async def get_account_balances(self) -> Dict[str, Dict[str, Decimal]]:
        ...
        for token, balance in balances.items():
            ...
            kuru_balances[token] = DotMap({}, _dynamic=False)
            kuru_balances[token]["total_balance"] = balance
            kuru_balances[token]["available_balance"] = balance

        # self.logger().debug("get_account_balances: end")

        return kuru_balances
        ...
        if active_order:
            ...
            if active_order.current_state != OrderState.CANCELED:
                ...
                if order_response:
                    ...
                    if order:
                        order_status = KuruOrderStatus.to_hummingbot(KuruOrderStatus.from_name(order.state))
                    else:
    ...
    async def _update_markets(self):
        ...
        if self._trading_pairs:
            for trading_pair in self._trading_pairs:
                ...
                self._markets_info[trading_pair] = market
        else:
            request = {
                "connector": self._connector,
                "chain": self._chain,
            ...
            for market in self._markets.values():
                ...
                self._markets_info[market["market"]] = market
    ...
    def _parse_trading_rule(self, trading_pair: str, market_info: Any) -> TradingRule:
        ...
        trading_rule = TradingRule(
            trading_pair=trading_pair,
            min_order_size=Decimal(market_info["minSizeStandardized"]),
            min_price_increment=Decimal(market_info["tickSize"]),
            min_base_amount_increment=Decimal(market_info["sizeIncrement"]),
            min_quote_amount_increment=Decimal(market_info["tickSize"]),
        )
    ...
    def _get_maker_taker_exchange_fee_rates_from_market_info(self, market_info: Any) -> MakerTakerExchangeFeeRates:
        ...
        output = MakerTakerExchangeFeeRates(
            maker=Decimal(0),
            taker=Decimal(market_info["takerFeeBps"]),
            maker_flat_fees=[],
            taker_flat_fees=[]
        )
    ...
    async def _update_order_status(self):
        async with self._locks.all_active_orders:
            ...
            for order in orders:
                ...
                try:
                    if response["orders"] is not None and len(response['orders']) and response["orders"][0] is not None:
                        ...
                        if updated_order["state"] in {
                            ...
                            self._publisher.trigger_event(event_tag=MarketEvent.OrderCancelled, message=message)
